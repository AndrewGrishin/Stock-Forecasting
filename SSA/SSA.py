...
class SSA:
    ...
    def forecast(
        ...
        """
        :param y: initial time series
        :param K: number of elements in row of Hankelian
        :param h: forecasting horizon
        :param r: Hankelian matrix rank
        :return: [y, forecast]: len(forecast) = h
        """
        ...
        for i in range(h):
            ...
            # r = np.linalg.matrix_rank(Y) # для информации
            Y = SSA.x_to_Hankelian(y, L)
            U, Lambda, Vt = np.linalg.svd(Y)
            V = Vt.T
            ...
            R = 1 / (1 - np.power(pi, 2).sum()) * Up @ pi.T
            f = np.array([np.dot(R,  y1[-L + 1:])])

            y = np.concatenate([y, f], axis= 0)
